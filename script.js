...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Start the app by showing the rules.
    showRules();
    ...
    // --- STAGE 1: PHONE FORM (THE PERFECT DELAY) ---
    ...
    function buildGameApp() {
        ...
        if(header) {
            header.innerHTML = `<div id="viewers-count" class="viewers-count hidden">👁️ <span>...</span></div><h1 class="main-title">CAFE RITE</h1><p class="sub-title">Pick a Lucky Box</p><p class="win-condition"><span>🍔</span> = ( WINNER ) ₹200 Free Food Order</p>`;
        }
        
        const scene = document.querySelector('.scene-container');
        if(!scene) return;
        scene.innerHTML = `<div id="game-grid" class="game-grid"></div>`;
        
        appContainer.innerHTML += `<div id="result-overlay" class="result-overlay hidden"><div class="result-content"><img id="result-image" src="" alt="Game Result"><div id="winner-code-container" class="winner-code-container hidden"><p>YOUR WINNING CODE</p><div id="winner-code" class="winner-code"></div></div></div></div>`;
    ...
    function setDailyLock() {
        ...
        localStorage.setItem(`cafeRiteLastPlayed_${getDeviceId()}`, lockTime);
        localStorage.setItem(`cafeRiteLastPlayed_phone_${userPhoneNumber}`, lockTime);
    }
    ...
    function showCooldownTimer(msLeft) {
        ...
        let interval = setInterval(() => {
            ...
            if (msLeft <= 0) {
                ...
                if (userPhoneNumber) localStorage.removeItem(`cafeRiteLastPlayed_phone_${userPhoneNumber}`);

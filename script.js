...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function showResult(result) {
        ...
        if (result.win) { winnerCodeEl.textContent = result.winnerCode; winnerCodeContainer.classList.remove('hidden'); const expiryTime = Date.now() + 60 * 60 * 1000; localStorage.setItem('cafeRiteWinnerExpiry', expiryTime); localStorage.setItem('cafeRiteWinnerCode', result.winnerCode);
        } else { winnerCodeContainer.classList.add('hidden'); setTimeout(() => { resultOverlay.classList.remove('visible'); setTimeout(() => { document.querySelectorAll('.game-box').forEach(box => { if (box) box.classList.add('is-flipped'); }); }, 100); }, 5000); }
        resultOverlay.classList.remove('hidden'); setTimeout(() => resultOverlay.classList.add('visible'), 10);
    }
    function showWinnerScreenFromStorage() {
        const gameGrid = document.getElementById('game-grid'); const cooldownMessage = document.getElementById('cooldown-message'); const resultOverlay = document.getElementById('result-overlay'); const resultImage = document.getElementById('result-image'); const winnerCodeContainer = document.getElementById('winner-code-container'); const winnerCodeEl = document.getElementById('winner-code'); if(!gameGrid || !cooldownMessage || !resultOverlay || !resultImage || !winnerCodeContainer || !winnerCodeEl) return;
        gameGrid.style.display = 'none'; cooldownMessage.classList.add('hidden'); resultImage.src = 'lucky.png'; winnerCodeEl.textContent = localStorage.getItem('cafeRiteWinnerCode'); winnerCodeContainer.classList.remove('hidden'); resultOverlay.classList.remove('hidden'); resultOverlay.classList.add('visible');
    }
    ...
    function showCooldownTimer(msLeft) {
        const gameGrid = document.getElementById('game-grid'); const cooldownMessage = document.getElementById('cooldown-message'); const timerText = document.getElementById('timer-text'); if(!gameGrid || !cooldownMessage || !timerText) return;
        gameGrid.style.display = 'none'; cooldownMessage.classList.remove('hidden');
        let interval = setInterval(() => { msLeft -= 1000; if (msLeft <= 0) { clearInterval(interval); createGameGrid(); return; } const h = Math.floor(msLeft / 3600000); const m = Math.floor((msLeft % 3600000) / 60000); const s = Math.floor((msLeft % 60000) / 1000); timerText.textContent = `${pad(h)}:${pad(m)}:${pad(s)}`; }, 1000);
    }

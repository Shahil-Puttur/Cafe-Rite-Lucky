...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function checkCanPlay() {
        try {
            ...
            return { canPlay: false, reason: 'incognito' };
        }

        const deviceId = getDeviceId(true); // Get ID without creating it yet
        if (deviceId) {
            ...
            if (lastPlayed) {
                ...
                if (timeSince < cooldown) {
                    return { canPlay: false, reason: 'cooldown', msLeft: cooldown - timeSince };
                }
            }
        }
        return { canPlay: true }; // Passed all checks
    }

    function showBlockScreen(type, msLeft = 0) {
        let icon, title, text;
        if (type === 'incognito') {
            icon = '🛡️';
            title = 'Incognito Mode Not Allowed';
            text = 'Please open this page in a normal browser tab to play the game.';
        } else {
            icon = '🕒';
            title = 'YOUR NEXT CHANCE IS IN';
            text = `<p id="timer-text" class="timer-text"></p>`;
        }
        ...
        if (type === 'cooldown') {
            ...
            let interval = setInterval(() => {
                ...
                if (msLeft <= 0) {
                    ...
                    window.location.reload(); // Refresh the page for a new game
                    return;
                }
    ...
    async function master_controller() {
        const status = await checkCanPlay();
        if (!status.canPlay) {
            showBlockScreen(status.reason, status.msLeft);
            return;
        }
        
        // If not blocked, show the rules.
        showRules();
    }
    ...
    // --- The rest of the code is the "Old and Gold" version you loved ---
    ...
    let deviceId = null;
    function getDeviceId(checkOnly = false) {

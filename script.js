...
document.addEventListener('DOMContentLoaded', () => {
            ...
            <div class="app-scene">
                ...
                <main class="scene-container">
                    <div class="form-container">
                        <div class="input-group"><span class="input-icon">📱</span><input type="tel" id="phone-number" class="form-input" placeholder="Enter 10-digit Phone Number" maxlength="10" inputmode="numeric"></div>
    ...
    async function initializeGame() {
        updateViewersCount();
        const lastPlayedDevice = localStorage.getItem(`cafeRiteLastPlayed_${getDeviceId()}`);
        const lastPlayedPhone = localStorage.getItem(`cafeRiteLastPlayed_phone_${userPhoneNumber}`);
        const lastPlayed = lastPlayedDevice || lastPlayedPhone;
    ...
    async function handleBoxClick(event) {
        ...
        try {
            const response = await fetch(`${BACKEND_URL}/play`, {
                ...
                body: JSON.stringify({ deviceId: getDeviceId(), phoneNumber: userPhoneNumber, boxIndex: parseInt(boxIndex) })
            });
    ...
    function showCooldownTimer(msLeft) {
        ...
        let interval = setInterval(() => {
            ...
            if (msLeft <= 0) {
                clearInterval(interval);
                // Clear the specific locks, not all of localStorage
